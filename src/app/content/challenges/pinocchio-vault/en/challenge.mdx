...
In this challenge, we'll build a simple lamport vault that demonstrates how to work with basic accounts, Program Derived Addresses (PDAs), and Cross-Program Invocation (CPI). If you're not familiar with Pinocchio, you should start by reading the [Introduction to Pinocchio](learn.blueshift.gg/en/courses/introduction-to-pinocchio) to familiarize with the core concepts that we're going to use in this program.
