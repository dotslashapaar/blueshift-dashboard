...
In this challenge, we'll build a simple lamport vault that demonstrates how to work with basic accounts, Program Derived Addresses (PDAs), and Cross-Program Invocation (CPI). If you're not familiar with Anchor, you should start by reading the [Instroduction to Anchor](learn.blueshift.gg/en/courses/introduction-to-anchor) to familiarize with the core concept that we're going to use in this program.

<ArticleSection name="Installation" id="intallation" level="h2" />
