...
**Note**: Every discriminator in your program must be unique. If two types share the same prefix the runtime can't tell them apart and Anchor's IDL generator will throw an error if it spots a collision. Additionally, know that using `[1]` as discriminator blocks you from ever adding `[1, 2, …]` later because it also starts with `1`.
