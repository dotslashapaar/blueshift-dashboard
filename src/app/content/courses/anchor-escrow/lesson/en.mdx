...
pub struct Account {
    ...
    pub data: Vec<u8>,
    /// the program that owns this account and that is allowed to mutate this account's lamports or data
    pub owner: Pubkey,
    /// `true` if the account is a program; `false` if it merely belongs to one
    pub executable: bool,
    /// the epoch at which this account will next owe rent (currently deprecated so set to `0`)
    pub rent_epoch: Epoch,
}
```

All accounts on Solana share the same base layout. What sets them apart is simply which program owns them since it has exclusive rights to add, delete, or modify the data stored inside.
...
Another important category of accounts are the Program Accounts: accounts owned by on-chain programs that are used store critical protocol or user state. The `Escrow` record we are going to create in this chapter is exactly this kind of account.

<ArticleSection name="Token Program" id="token-program" level="h2" />

The Token Program, sart of the Solana Program Library (SPL), is the built-in toolkit for minting and moving any asset that isn't native SOL. It has instructions to create tokens, mint new supply, transfer balances, burn, freeze, and more.
...
- **Token Account**: holds a balance of that mint for a particular owner. Only the owner can reduce the balance (transfer, burn, etc.), but anyone can send tokens in, increasing it.
...
We've already covered Anchor's core macros in the previous unit; now we'll put one of its most powerful to work. In this section we'll use `#[account]` to define a custom data structure and store it inside an account that our program owns.

### `#[account]`

In the vault example, our PDA was just an empty System account with a deterministic address. When PDA are owned by our program they unlock its second super-power: we can write arbitrary data into it. Anchor makes that painless.
...
Here are some of the field used in the `#[account]` macro, beyond the `seeds` and `bump` fields that we have already covered, and what they do:
...
Then, once we created it, we can access the data of the account like this:
...
These token-related accounts follow the same `init` pattern we used earlier. Because Anchor already knows their fixed byte size, we don't need to supply a space value but just tell Anchor who pays the rent.
...
When `has_one` isn’t a fit, because maybe the field name differs, you can write any boolean expression with constraint. If the expression is false, Anchor raises the error you specify.
...
An escrow lets one user lock up Token A in exchange for Token B from a second user. Our contract therefore boils down to three instructions:
...
Add the crates we’ll need, enabling Anchor’s init-if-needed helper so we can lazily create token accounts:
...
`state.rs` is straightforward; we already know how `#[account]` works, and we already seen this structure in the past section:
...
We could pack in more info, but extra bytes mean extra rent. Storing only the essentials keeps deposits cheap while still letting the contract enforce every rule it needs.
