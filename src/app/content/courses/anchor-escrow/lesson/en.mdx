...
Anchor needs a quick way to tell one account from another. It does that with an 8-byte discriminator; a tiny prefix stored at the start of every serialized struct that is generated using this snippet: `sha256("account:<StructName>")[0..8]`

When Anchor reads raw bytes, it peeks at the first 8 bytes, matches them to a table of known discriminators, and then deserializes into the right Rust struct or routes to the right handler. And the hash-based approach guarantees each type in your program has a unique marker 

##### Custom Discriminators

Starting with Anchor `v0.31` you can override the auto-generated hash and give an account or instruction its own discriminator directly in the `#[account]` or `#[instruction]` macro. 

```rust
#[account(discriminator = 0)]              // single-byte
pub struct Escrow { … }
...
**Note**: Every discriminator in your program must be unique. If two types share the same prefix the runtime can’t tell them apart and the Anchor’s IDL generator will throw an error if it spots a collision. Additionally, know that using `[1]` as discriminator blocks you from ever adding `[1, 2, …]` later because it also starts with `1`.

For this `Escrow` program we’ll tag everything with a single-byte discriminator:
- `Escrow` account: 0x00
- `make` instruction: 0x00
- `take` instruction: 0x01
- `refund` instruction:` 0x02

The default 8-byte hashes would work fine; but it shows how a custom, shorter discriminator trims a few bytes of on-chain storage.
