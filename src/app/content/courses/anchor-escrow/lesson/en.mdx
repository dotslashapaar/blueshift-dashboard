...
When we talk about Token Program Accounts, what we mean is an account where the `owner` is the Token Program. Unlike a System Account whose data field is empt, a Token Program Account can be either a **Mint** or a **Token** account. We use discriminators to distringuish between them.
...
An important feature of the `#[account]` macro (which is also relevant to intructions) is the use of discriminators. These are unique identifiers that help Anchor distinguish between different accounts or instructions within your program. Let's explore this concept before looking at the `Escrow` struct.

## Discriminators

Anchor needs a quick way to tell one account from another. We do this with either a Default Discriminaotr, or a Custom Discriminator.
...
<ArticleSection name="Escrow" id="escrow" level="h2" />
