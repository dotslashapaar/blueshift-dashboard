...
In Anchor, instructions are defined using the `#[program]` module and individual instruction functions. Each instruction function must be marked with the `#[instruction]` attribute. Here's the basic structure:
...
Cross Program Invocations (CPI) refer to the process of one program invoking instructions of another program, which enables the composibility of Solana programs. Anchor provides a convenient way to make CPIs through the `CpiContext` and program-specific builders.
