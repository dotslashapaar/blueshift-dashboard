...
While most Solana developers lean on Anchor, there are plenty of good reasons to write a program without it. Perhaps you need finer-grained control over every account field, or you’re chasing maximum performance free from macro overhead. 

Native development is more demanding, yet in this chapter you’ll learn to craft a Solana program from scratch with Pinocchio; a lightweight library that lets you skip external frameworks and own every byte of your code.

# Pinocchio 101

Pinocchio is a minimalist Rust library that lets you craft Solana programs without pulling in the heavyweight `solana-program` crate. It works by treating the incoming transaction payload—accounts, instruction data, everything; as a single byte slice and reading it in-place via zero-copy techniques. 

That one design choice unlocks three big wins:
...
The project was started by [Febo](https://x.com/0x_febo) at Anza with core contribution from the Solana ecosystem and the Blueshift team and lives here: https://github.com/anza-xyz/pinocchio. 
...
Switching to Native, means we don't have that luxury anymore and that we will need to:
...
In Anchor, the `#[program]` macro hides a lot of wiring. Under the hood it builds an 8-byte discriminator for every instruction by hashing `"global:"` + function_name with SHA-256:
...
3. Each instruction’s `try_from` impl validates and deserializes its inputs.
...
**What’s happening**
1. Destructure the input slice into named variables—fail fast if the caller supplies too few accounts.
...
2. Delegates validation to the individual TryFrom impls.
...
The result? Anchor-style ergonomics, but fully native—explicit, predictable, and fast.

### Cross-Program Invocations (CPIs) and Signer Seeds

As said before, Pinocchio has some helper crates like `pinocchio-system` and `pinocchio-token` that make it super easy to perform Cross-Program Invocations (CPIs) to these native programs. 
...
When the caller must be a Program-Derived Address, Pinocchio keeps the API equally concise:
...
**Here's what happens`:
1. `Seeds` Build an array of Seed objects that match the PDA derivation.
2. `Signer` Wrap those seeds in a Signer helper.
3. `invoke_signed` Call the CPI, passing the signer array so the PDA can authorize the transfer.
...
```
# create workspace
cargo new blueshift_vault --lib
cd blueshift_vault
```

Add Pinocchio to Cargo.toml:

```
[dependencies]
pinocchio         = "x.x.x"
pinocchio-system  = "x.x.x"
pinocchio-token   = "x.x.x"
```
