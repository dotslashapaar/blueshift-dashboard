...
# Bytes by Bytes: The Anatomy of a Crateless Program

![Image Test](/graphics/anchor-101.png)

If you have written a Solana program, you definetely have used a crate like `solana_program`, `pinocchio` or a framework that used one of these crates under the hood like `anchor`, `steel`...
...
- `account_info`: that many raw account records, one after another.
...
Everything will live in `lib.rs`. At a glance, the file will contain Syscall Shims (tiny, #[repr(C)] structs that mirror the raw syscall layouts we'll need), and offsets as constants (to make it easier to work with raw data).
...
We continue by checking each single account, the layout offsets we defined earlier let us sanity-check every flag without deserialising the full struct.

For the first account, the signer, this are the checks that we perform: 
...
For the second account, the vault, this are the checks that we perform:
- Writable (we'll change its lamports)
...
For the last account, the system program. this are the checks that we perform: 
- Must be executable
- Its key must equal pinocchio_system::ID
...
if *(input.add(ACCOUNT_3_OFFSET + KEY_OFFSET) as *const [u8; 32]) != pinocchio_system::ID {
...
AFter that we hash the hash the owner key using the `sol_sha256` syscall, with the bump, the program ID and the unique string
"ProgramDerivedAddress". We compare the result with the vault key we stashed while we were performing account checks previously.
...
The System Program uses a four-byte discriminator (for transfer that value is 2) and require an additional 8 bytes that rapresent the amount that we want to transfer from the `from_account` to the `to_account`

We then need to build a tiny array of two SolAccountMeta values: from = owner (signer, writable), to = vault (writable). That we wrap with the `instruction_data` we just created a `SolInstruction` struct like this:
...
- Four-byte discriminator (2 for transfer)
