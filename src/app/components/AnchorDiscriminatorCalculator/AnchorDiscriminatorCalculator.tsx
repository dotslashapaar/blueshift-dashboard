...
export const AnchorDiscriminatorCalculator = ({
  ...
  return (
    <div className="mx-auto p-6 bg-background-card border border-border rounded-lg flex flex-col gap-y-4 group enabled:hover:cursor-pointer rounded-xl transition duration-200 enabled:hover:bg-background-card-foreground/50">
      <div className="text-2xl font-bold">Anchor Discriminator Calculator</div>
      <label htmlFor="seed" className="block text-md font-medium text-gray-400 mb-1">
        Account/Instruction Name
      </label>
      <div className="w-full group focus-within:outline transition outline-transparent focus-within:outline-border-active relative h-[50px] px-3 bg-card border border-border bg-background-card rounded-xl flex items-center gap-x-3">
        <input
          id="seed"
          type="text"
          value={seed}
