      ...
      <div className="relative flex flex-col gap-y-4 w-full overflow-hidden px-1.5 pt-1.5 pb-12 border rounded-2xl border-border bg-background-card">
        ...
        >
          ...
          <Button
            ...
            icon={"Upload"}
            size="lg"
            label={isCodeRunning ? t("challenge_page.running_program_btn") : t("challenge_page.run_program_btn")}
        ...
        {runnerLogs.length > 0 && (
           <div className="flex flex-col gap-y-2 px-2 mt-4">
             <div className="flex flex-col gap-y-4 items-start overflow-hidden bg-background pt-4 px-1 pb-1 rounded-xl border border-border">
               <HeadingReveal
                 baseDelay={0.1}
                 text="TYPESCRIPT EXECUTION LOGS"
                 headingLevel="h3"
                 className="font-mono px-3 text-base"
                 color="#00ACC1"
               />
               <div className="px-2 w-full"><Divider /></div>
               <div className="flex flex-col gap-y-1 items-start px-3 overflow-x-auto pr-5 pb-2 w-full max-h-80 custom-scrollbar font-geist-mono text-xs">
                 {runnerLogs.map((log, index) => (
                   <motion.div
                     ...
                     className={classNames(
                       "text-start text-nowrap py-0.5",
                       {
                         'text-red-400 font-bold': log.type === 'ERROR' || log.type === 'EXECUTION_ERROR' || log.type === 'WORKER_ERROR' || log.type === 'VERIFICATION_ERROR',
                         'text-yellow-400': log.type === 'WARN',
                         'text-sky-300': log.type === 'INFO' || log.type === 'SYSTEM',
                         'text-slate-400': log.type === 'DEBUG',
                         'text-secondary': log.type === 'LOG',
                       }
                     )}
                   >
                     <span className="font-semibold mr-1.5">{`[${log.timestamp.toLocaleTimeString()}]`}</span>
                     <span className={`font-bold ${ log.type === 'SYSTEM' ? 'text-purple-300' : 
                                                 log.type === 'LOG' ? 'text-green-300' : ''}`}>{`[${log.type}]`}</span>
                     <span className="ml-1.5">{Array.isArray(log.payload) ? log.payload.map(p => typeof p === 'object' ? JSON.stringify(p) : String(p)).join(' ') : String(log.payload)}</span>
                   </motion.div>
                 ))}
               </div>
             </div>
